...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        tbody.innerHTML = drugs.length ? drugs.map(d => {
            const status = d.stok_obat <= d.min_stock
            ...
            return `<tr>
                ...
                <td>${d.nama_obat || '-'}</td>
                <td>${d.unit || '-'}</td>
                <td class="font-bold">${d.stok_obat}</td>
                <td>Rp ${parseInt(d.harga_jual_eceran || 0).toLocaleString()}</td>
                <td>${d.expiry_date ? d.expiry_date.slice(0,10) : '-'}</td>
                <td>${status}</td>
                <td><button class="btn btn-sm btn-outline-info p-1.5" onclick="alert('Detail ${d.kode_obat}')"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></button></td>
            </tr>`;
        }).join('') : '<tr><td colspan="8" class="text-center py-4 text-slate-400">Tidak ada data obat</td></tr>';
