    ...
    <div class="max-w-full px-8 py-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10 pb-8 border-b border-white/10">
            ...
            <div class="space-y-4">
                ...
                <ul id="footerInfo" class="space-y-3 text-[13px] text-white/60">
                    <li class="flex gap-2.5 items-start">
                        ...
                        <span id="footerAlamat">Memuat...</span>
                    </li>
                    <li class="flex gap-2.5 items-center">
                        ...
                        <span id="footerTelepon">Memuat...</span>
                    </li>
                    <li class="flex gap-2.5 items-center">
                        <svg class="w-4 h-4 text-[#4154f1] flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                        </svg>
                        <span id="footerEmail">Memuat...</span>
                    </li>
                    <li class="flex gap-2.5 items-center">
                        <svg class="w-4 h-4 text-[#4154f1] flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <span id="footerJam">Memuat...</span>
                    </li>
                </ul>
                <script>
                fetch('/api/settings').then(r => r.json()).then(json => {
                    const s = json.data || {};
                    if (s.alamat) document.getElementById('footerAlamat').textContent = s.alamat;
                    if (s.telepon) document.getElementById('footerTelepon').textContent = s.telepon;
                    if (s.email) document.getElementById('footerEmail').textContent = s.email;
                    if (s.jam_buka_senin_jumat) document.getElementById('footerJam').textContent = 'Senin - Jumat: ' + s.jam_buka_senin_jumat;
                }).catch(() => {});
                </script>
            </div>
